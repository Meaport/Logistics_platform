/*
  # Transport Service Schema Setup

  1. New Tables
    - `vehicles` - Vehicle management with capacity and status tracking
    - `shipments` - Shipment tracking with origin/destination and status
    - `route_logs` - GPS tracking and route logging for shipments

  2. Security
    - Enable RLS on all tables
    - Add policies for authenticated users and public tracking
    - Create appropriate indexes for performance

  3. Features
    - Automatic tracking number generation
    - Status tracking for vehicles and shipments
...
-- Create triggers for updated_at (without IF NOT EXISTS as it's not supported)
